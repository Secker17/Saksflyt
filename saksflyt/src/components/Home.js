...
function Home({ user }) {
  ...
  function addCase(formData) {
    const newCase = {
      id: `${new Date().getFullYear()}-${String(cases.length + 1).padStart(4, '0')}`,
      ...
      initials: formData.person.slice(0, 2).toUpperCase(),
  ...
  function scrollToCases() {
    document.getElementById('case-table')?.scrollIntoView({ behavior: 'smooth' });
  }
  ...
  return (
    <div className="dashboard">
      ...
      {selectedCase && (
        <CaseDetails item={selectedCase} onClose={() => setSelectedCase(null)} />
      )}
      </div>
