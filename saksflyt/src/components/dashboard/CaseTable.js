...
function CaseTable({ cases, hasCases, onNewCase, selectedCase, onSelect }) {
  return (
    <section className="table-card" id="case-table">
      <table>
        <thead>
          <tr><th>Saksnr</th><th>Tittel</th><th>Kategori</th><th>Prioritet</th><th>Status</th><th>Ansvarlig</th><th>Opprettet</th><th></th></tr>
        </thead>
        <tbody>
          {cases.map((item) => (
            <tr
            ...
            >
              ...
              <td><span className="person"><span className="avatar">{item.initials}</span>{item.person}</span></td>
              <td>{item.date}</td>
              <td className="menu-dots"><MoreHorizontal /></td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      {cases.length === 0 && (
        <div className="empty-message">
          ...
          <p>{hasCases ? 'Prøv et annet søk eller endre filtrene.' : 'Opprett den første saken for å komme i gang.'}</p>
          {!hasCases && <button onClick={onNewCase}><CirclePlus /> Opprett sak</button>}
        </div>
      )}
    </section>
  );
